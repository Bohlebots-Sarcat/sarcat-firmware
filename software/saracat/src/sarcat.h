...
/*
 * Festlegungen der Motoren im bot-type-Betrieb
...
class BohleBots
{
  ...
  void servo(int pos)  // sinnvollerweise von -100 bis 100
  {
    ...
    void i2csync()
    {
      ...
      if (hatKompass==true) kompass_wert=kompass_lesen();
      else kompass_wert=0;
      
      int irpaket=0;
      Wire.requestFrom(IR_ADRESSE, 1);
      if(Wire.available()) irpaket= Wire.read();
      ball_richtung=(irpaket%100)-10;    
      int zone=irpaket/100;                       
      if (zone<1) ball_max=0;
      else ball_max=1;    
      if (ball_max<1)     ball_richtung=0;
            
      // Lichtschranke (nicht wirklich i2c...)
      int licht = analogRead(INPUT3);
...
{     
  ...
  void pixy_read()
  {
    ...
    if ((seheTor==false) && (hatKompass)) torRichtung  =  kompass() ; //wenn die Kamera nicht funktioniert -> Kompass   
  }  
